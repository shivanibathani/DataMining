...
os.chdir("/Users/shivani/Desktop/SHIVU/DM project/Incomplete datasets(1)/Data 1")
filenames = glob.glob('*.csv')
fcount = 0
result = pd.read_excel(
    '/Users/shivani/Desktop/SHIVU/DM project/Table-NRMS.xlsx', header=None)

for filename in filenames:
    start = timeit.default_timer()
    missing_dataset = pd.read_csv(filename, header=None)
    learning_dataset = missing_dataset.dropna()
    ImputedDataset = missing_dataset
    original = pd.read_csv(
        '/Users/shivani/Desktop/SHIVU/DM project/Complete datasets(1)/Data_1.csv')

    if(learning_dataset.empty):
        result.loc[fcount, 0] = filename
        result.loc[fcount, 1] = "Error"
        result.loc[fcount, 2] = 0
        result.to_excel(
            "/Users/shivani/Desktop/SHIVU/DM project/Table-NRMS.xlsx", index=False, header=False)
        print("File: ", filename)
        print("Time: 0")
        print("NRMS: Error")
        fcount = fcount + 1
        ...
        learning_dataset = learning_dataset.apply(
        ...
        for i in range(len(missing_dataset.columns)-1):
            X_Train_df = learning_dataset.iloc[:,
                                               learning_dataset.columns != i]
            y_Train_df = learning_dataset.iloc[:, lambda learning_dataset: [i]]
        ...
        for i in range(len(original.index)):
            for j in range(len(original.columns)-1):
                ...
                total = total + \
                    (float(original.iloc[i, j])*float(original.iloc[i, j]))

        NRMS = math.sqrt(difference_sum)/math.sqrt(total)

        result.loc[fcount, 0] = filename
        result.loc[fcount, 1] = NRMS
        result.loc[fcount, 2] = total_time
        result.to_excel("/Users/shivani/Desktop/SHIVU/DM project/Table-NRMS.xlsx",
